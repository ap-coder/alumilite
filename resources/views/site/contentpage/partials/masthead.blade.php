    ...
    <div class="row">
      <div class="col-md-10 order-2 order-md-1 align-self-center p-static">
          @if ($page->show_title && $page->title)
            <h1 class="text-capitalize font-weight-bold text-8 mb-1 {{ $page->title_style ?? 'text-primary bg-light' }} mt-4 p-2">{!! $page->title ?? '' !!}</h1>
          @endif
          @if ($page->show_tagline && $page->sub_title)
            <span style="display: table;" class="h4 text-uppercase text-capitalize font-weight-bold sub-title mb-1 {{ $page->tagline_style ?? 'text-secondary  bg-dark' }} p-2 ">
              {!! $page->sub_title ?? '' !!}
            </span>
          @endif
    ...
    <div class="row">
      <div class="col-md-12 align-self-center p-static order-2 text-center">
        ...
        @if ($page->show_title_box)
          ...
          @endif
          @if ($page->show_tagline && $page->sub_title)
            <span style="display: table;" class="sub-title {{ $page->tagline_color ?? 'text-secondary' }}">{!! $page->sub_title ?? '' !!}</span>
          @endif
          @if ($page->show_featured_content && $page->featured_image_content)
