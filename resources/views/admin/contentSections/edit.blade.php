...
<div class="card">
    <div class="card-header">
        {{ trans('global.edit') }} {{ trans('cruds.contentSection.title_singular') }}
    </div>

    <div class="card-body">
        <form method="POST" action="{{ route("admin.content-sections.update", [$contentSection->id]) }}" enctype="multipart/form-data">
            @method('PUT')
            @csrf
            ...
            <div class="form-group">
                ...
                <input class="form-control {{ $errors->has('order') ? 'is-invalid' : '' }}" type="number" name="order" id="order" value="{{ old('order', $contentSection->order) }}" step="1">
                @if($errors->has('order'))
            ...
            <div class="form-group">
                ...
                <select class="form-control select2 {{ $errors->has('pages') ? 'is-invalid' : '' }}" name="pages[]" id="pages" multiple>
                    @foreach($pages as $id => $page)
                        <option value="{{ $id }}" {{ (in_array($id, old('pages', [])) || $contentSection->pages->contains($id)) ? 'selected' : '' }}>{{ $page }}</option>
                    @endforeach
                </select>
                @if($errors->has('pages'))
